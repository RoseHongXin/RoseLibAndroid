...
public class WheelView extends ScrollView {
    ...
    public void setItems(List<String> list) {
        if (null == items) items = new ArrayList<String>();
        items.clear();
        items.addAll(list);
        // 前面和后面补全
    ...
    private void initData() {
        ...
        for (String item : items) views.addView(createView(item));
        refreshItemView(0);
    }
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        refreshItemView(t);
        if (t > oldt) scrollDirection = SCROLL_DIRECTION_DOWN;
        else scrollDirection = SCROLL_DIRECTION_UP;
    }
    ...
    private void refreshItemView(int y) {
        ...
        for (int i = 0; i < childSize; i++) {
            ...
            if (position == i) itemView.setTextColor(Color.parseColor("#0288ce"));
            else itemView.setTextColor(Color.parseColor("#bbbbbb"));
        }
    }
    ...
    public void setBackgroundDrawable(Drawable background) {
        if (viewWidth == 0) viewWidth = ((Activity) context).getWindowManager().getDefaultDisplay().getWidth();
        if (null == paint) {
            paint = new Paint();
